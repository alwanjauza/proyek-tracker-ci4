            ...
            <div class="col-12 grid-margin stretch-card">
                <div class="card">
                    <div class="card-body">
                        ...
                        <form class="forms-sample" action="/admin/menus/tim/update/<?= $timIt->id_tim ?>" method="post">
                            <?= csrf_field(); ?>
                            <div class="form-group">
                                <label for="nama_tim">Nama Tim</label>
                                <input type="text" class="form-control <?= (session()->getFlashdata('errors')['nama_tim'] ?? '') ? 'is-invalid' : '' ?>" id="nama_tim" name="nama_tim" placeholder="Nama Tim" value="<?= $timIt->nama_tim ?>">
                            ...
                            <div class="form-group">
                                ...
                                <select name="anggota_tim[]" id="anggota_tim[]" class="form-control js-example-basic-multiple" multiple="multiple" style="width:100%">
                                    <?php
                                    $anggotaTim = json_decode($timIt->anggota_tim, true);
                                    ?>
                                    <?php foreach ($users as $user) : ?>
                                        <option value="<?= $user->id ?>" <?= in_array($user->id, $anggotaTim) ? 'selected' : '' ?>>
